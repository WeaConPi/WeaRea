...
class comp extends React.Component<IAppProps, any> {
  ...
  handleChangeDate = newDate => this.props.actions.fetchDay(moment(newDate));

  handleHourChange = hour => () => this.props.actions.selectHour(hour);
  ...
  renderLine = () => (
    <div className="pt-button-group ">
      {this.props.hours.toIndexedSeq().map((hour, key) => (
        <button
          key={key}
          className={`pt-button ${key === this.props.selectedHour
            ? 'pt-active'
            : ''}`}
        ...
        <StyledBody>
          <div>
            ...
            <HourDetail
              ...
              hour={this.props.hours.get(this.props.selectedHour)}
            />
          </div>
          <div>
            <h2>Daily summary </h2>
...
export const DayDetail = connect(
  mapStateToProps,
  mapDispatchToProps,
)(comp as any);
